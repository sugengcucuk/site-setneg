...
class Ion_auth
{
	...
	public function is_allowed($id_menu,$action) 
	{
		$id_user = $this->session->userdata('user_id');
		$roles = $this->session->userdata('roles');
		
		if($action=="view"){$str_action="AccView";}else if($action=="create"){$str_action="AccCreate";}else if($action=="update"){$str_action="AccUpdate";}else if($action=="delete"){$str_action="AccDelete";}
		//print_r($roles);exit;
		$allow=false; 
		...
		//return $this->ion_auth_model->check_is_allowed($roles,$id_menu,$str_action);
	}
	
	public function is_parent_menu_allowed($array_menu) 
	{
		$id_user = $this->session->userdata('user_id');
		$roles = $this->session->userdata('roles');
		
		for($x = 0; $x < count($array_menu); $x++) {
			$allow=false; 
			foreach ($roles as $role) {
				$query = $this->db->select("AccView")
							  ->where('RoleID', $role)
							  ->where('MenuID', $array_menu[$x])
							  ->limit(1)
							  ->order_by('RoleID', 'desc')
							  ->get('t_role_menu');
	...
	public function is_old_password_match($identity,$old_password)    //changed $email to $identity 
	{
		return $this->ion_auth_model->hash_password_db($identity, $old_password);
	}
