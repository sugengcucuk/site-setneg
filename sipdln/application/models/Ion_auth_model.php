...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email,Status, UserID,last_login,nama, password')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('UserID', 'desc')
		                  ->get($this->tables['users']);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.NamaRole, '.$this->tables['groups'].'.deskripsi')
		                ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
		                ->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.RoleID')
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'email'                => $user->email,
		    'user_id'              => $user->UserID, //everyone likes to overwrite id so we'll use user_id
		    'old_last_login'       => $user->last_login,
			'name'  			   => $user->nama,
			'roles'  			   => $this->get_user_roles($user->UserID),
			'user_detail'  		   => $this->get_user_detail($user->UserID)
		);
	...
	public function get_user_detail($UserID){
    	$this->db->select('m_user.nama,m_user.email,m_user.username,m_unit_kerja_institusi.Name as unit_kerja,r_institution.Nama as instansi');
    	$this->db->from('m_user');
		$this->db->join('r_institution','r_institution.ID = m_user.instansi','left'); 
		$this->db->join('m_unit_kerja_institusi','m_unit_kerja_institusi.ID = m_user.unitkerja','left');  
	...
	public function get_user_roles($UserID){
		...
		{
			$array[] = $row->RoleID; // add each role id to the array 
		}
	    //print_r($array);exit;
    	return $array;
	}
	
	public function check_is_allowed($roles,$id_menu,$str_action) 
	{
		$allow=false;
		foreach ($roles as $role) {
			$query = $this->db->select($str_action)
		                  ->where('RoleID', $role)
